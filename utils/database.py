...
def init_csv_files():
    ...
    files_structure = {
        ...
        OD_REQUESTS_CSV: ['request_id', 'student_id', 'date', 'start_time', 'end_time', 'reason', 
...
def get_attendance(student_id, detailed=False):
    ...
    try:
        if not os.path.exists(ATTENDANCE_CSV):
            return [] if detailed else {'total': 0, 'present': 0, 'absent': 0, 'percentage': 0}
